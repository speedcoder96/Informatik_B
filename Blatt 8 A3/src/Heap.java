 ...
 * @param <E>
 *
 * Implementing serializable in order to be able to serialize
 */
public class Heap<E> implements Serializable, Iterable<E> {

   private static final int DEFAULT_INITIAL_CAPACITY = 11;

   /**
    * Defines the serializable version of this class.
    */
   private static final long serialVersionUID = 1L;

   /*
    * The heap as Object[], it is not possible to instantiate a generic array.
    * Make object array of heap transient in order to be able to serialize not in
    * the default way.
    */
   ...
   private void writeObject(ObjectOutputStream outStream) throws IOException {
      //writes the size of the heap on the ObjectOutputStream in order to
      //be able to reconstruct the heap
      outStream.defaultWriteObject();
      for(Object element : heap) {
         outStream.writeObject(element);
      }
   }

   /**
    * Build the heap from the deserialized version delivered by the
    * ObjectInputStream.
    * @param inStream the input stream delivering the deserialized version
    * @throws IOException if an IO error occurs
    * @throws ClassNotFoundException if the serializable class doesn't exists
    *    (in terms of existing or serializable version difference)
    */
   private void readObject(ObjectInputStream inStream) throws IOException, ClassNotFoundException {
      inStream.defaultReadObject();
      heap = new Object[size];
      for(int i = 0; i < size; i++) {
         heap[i] = inStream.readObject();
      }
   }

   /**
    * For testing.
    * @return a iterator that is able to iterate over the heap
    */
   @Override
   ...
   /**
    * HeapIterator to be able to iterate over the heap.
    * For testing.
    * @param <E>
    */
   private static class HeapIterator<E> implements Iterator<E> {
